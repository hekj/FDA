...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        # self.map_ft_hist = nn.Linear(768*3, 768)
    ...
    def _cand_pano_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            for j, cc in enumerate(ob['candidate']):
                # cc['feature']: (2308, 0)
                # self.args.image_feat_size: 768
                cand_img_fts.append(cc['feature'][:self.args.image_feat_size*rgb_low_high_num])
                cand_ang_fts.append(cc['feature'][self.args.image_feat_size*rgb_low_high_num:])
            ...
            # add [STOP] feature
            cand_img_fts.append(np.zeros((self.args.image_feat_size*rgb_low_high_num,), dtype=np.float32))
            ...
            cand_pano_img_fts = np.concatenate([cand_img_fts, pano_fts[:, :self.args.image_feat_size*rgb_low_high_num]], 0)
            cand_pano_ang_fts = np.concatenate([cand_ang_fts, pano_fts[:, self.args.image_feat_size*rgb_low_high_num:]], 0)
    ...
    def _history_variable(self, obs):
        ...
        hist_img_feats = np.zeros((len(obs), self.args.image_feat_size*rgb_low_high_num), np.float32)
        for i, ob in enumerate(obs):
            hist_img_feats[i] = ob['feature'][ob['viewIndex'], :self.args.image_feat_size*rgb_low_high_num]
        hist_img_feats = torch.from_numpy(hist_img_feats).cuda()

        if self.args.hist_enc_pano:
            hist_pano_img_feats = np.zeros((len(obs), self.args.views, self.args.image_feat_size*rgb_low_high_num), np.float32)
            hist_pano_ang_feats = np.zeros((len(obs), self.args.views, self.args.angle_feat_size), np.float32)
            for i, ob in enumerate(obs):
                hist_pano_img_feats[i] = ob['feature'][:, :self.args.image_feat_size*rgb_low_high_num]
                hist_pano_ang_feats[i] = ob['feature'][:, self.args.image_feat_size*rgb_low_high_num:]
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        txt_embeds = self.vln_bert(**language_inputs)  # VLNBertCMT, from models.model_TDSTP import VLNBertCMT
