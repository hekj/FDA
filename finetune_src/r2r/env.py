...
class EnvBatch(object):
    ...
    def getStates(self, step=None):
        ...
        for i, sim in enumerate(self.sims):
            ...
            # self.feat_db: data_utils -> ImageFeaturesDB()
            feature = self.feat_db.get_image_feature(state.scanId, state.location.viewpointId, i, step)
            # print("in env.py getStates(): ", step)
...
class R2RBatch(object):
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        obs = []
        for i, (feature, state) in enumerate(self.env.getStates(step=t)):
            # print("in env.py get_obs(): ", t)
            item = self.batch[i]
            ...
            # [visual_feature, angle_feature] for views: visual_feature (2304), angle_feature (4)
