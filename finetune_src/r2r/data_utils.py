...
class ImageFeaturesDB(object):
    def __init__(self, img_ft_file, img_ft_file_2, image_feat_size):
        self.image_feat_size = image_feat_size
        self.img_ft_file = img_ft_file
        self.img_ft_file_2 = img_ft_file_2
    ...
    def get_image_feature(self, scan, viewpoint, idx=None, step=None):
        key = '%s_%s' % (scan, viewpoint)

        if key in self._feature_store:

            #  in turn (determine whether to use original features or enhanced features based on the order within the batch)
            if scan in self.val_unseen or scan in self.test:
                ft = self._feature_store[key]
            elif idx % 2 == 0:
                ft = self._feature_store[key]  # orignal features
            else:
                ft = self._feature_store_2[key]  # FDA enhanced features

            # # #  in turn (determine whether to use original features or enhanced features based on the step order)
            # if scan in self.val_unseen or scan in self.test:
            #     ft = self._feature_store[key]
            # elif step % 2 == 1:
            #     ft = self._feature_store[key]
            ...
            with h5py.File(self.img_ft_file, 'r') as f:
                # RGB ft
                ft_rgb = f[key][...][:, :self.image_feat_size].astype(np.float32)
                ft = ft_rgb
                self._feature_store[key] = ft

            with h5py.File(self.img_ft_file_2, 'r') as f:
                ft_fda = f[key][...][:, :self.image_feat_size].astype(np.float32)
                self._feature_store_2[key] = ft_fda
...
def load_instr_datasets(anno_dir, dataset, splits):
    ...
    for split in splits:
        if "/" not in split:  # the official splits
            ...
            with open(os.path.join(anno_dir, 'prevalent_aug_train_enc.json')) as f:
                new_data = json.load(f)
